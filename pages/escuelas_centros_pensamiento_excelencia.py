...
dash.register_page(__name__, path='/escuelas-centros-pensamiento-excelencia')

f = open("file.txt", "r")
token = f.readline()
e = open("environment.txt", "r")
environment = e.readline()
url = environment + "/reporte_cifras/buscarCifras?area_param=Investigación y Creación Artística&programa_param=Despliegue de la investigación &actividad_param=Escuelas y centros de pensamiento y de excelencia"
...
layout = html.Div([
    ...
    dbc.Nav(
        [
            ...
            dbc.NavItem(dbc.NavLink(
                "Escuelas y centros de pensamiento y de excelencia", active=True, href="/escuelas-centros-pensamiento-excelencia")),
            dbc.NavItem(dbc.NavLink(
        ...
        [
            dbc.Row(
                [
                    dbc.Col(html.Div([
                        dcc.Dropdown(
                            id="facultad_escuelas_centros_pensamiento_excelencia",
                    ...
                    dbc.Col(html.Div([
                        dcc.Dropdown(
                            id="anio_escuelas_centros_pensamiento_excelencia",
                    ...
                    dbc.Col(
                        html.Div([
                            dash_table.DataTable(
                                ...
                                id='logros_tabla_escuelas_centros_pensamiento_excelencia',
                            ),
                        ], style={'paddingTop': '2%'})
                    )
                ]
...
@callback(
    Output("logros_tabla_escuelas_centros_pensamiento_excelencia", "data"),
    [Input("facultad_escuelas_centros_pensamiento_excelencia", "value"), Input("anio_escuelas_centros_pensamiento_excelencia", "value")])
def logros_alcanzados_escuelas_centros_pensamiento_excelencia(facultad, anio):
    if facultad or anio:
        if not anio:
            df = data
