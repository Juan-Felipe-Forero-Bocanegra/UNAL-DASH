...
dash.register_page(
    __name__, path='/formulacion-ejecucion-proyectos-y-convenios-laboratorios')

f = open("file.txt", "r")
token = f.readline()
e = open("environment.txt", "r")
environment = e.readline()
url = environment + "/reporte_cifras/buscarCifras?area_param=Gestión de Laboratorios&programa_param=Proyectos de laboratorios&actividad_param=Acompañamiento para formulación, ejecución y seguimiento de proyectos y convenios"
    ...
    if c['informeActividadDetalle']['orden'] == 1:
        ...
        for a in c['informeActividadDetalle']['listaDatoListaValor']:
            ...
            if a['actividadDatoLista']['orden'] == '2':
                if a['indice'] == j:
                    o['Laboratorios participantes'] = a['cifra']
                    i += 1
            if i == 2:
                list.append(o)
                i = 0
...
layout = html.Div([
    ...
    dbc.Nav(
        [
            dbc.NavItem(dbc.NavLink("Proyectos y convenios", active=True,
    ...
    dcc.Graph(id="graph_valor_ejecutado_recursos_facultad_proyectos_y_convenios_laboratorios",
        ...
        [
            dbc.Row(
                [
                    dbc.Col(html.Div([
                        dcc.Dropdown(
                            id="facultad_proyectos_y_convenios_laboratorios",
                    ...
                    dbc.Col(html.Div([
                        dcc.Dropdown(
                            id="anio_proyectos_y_convenios_laboratorios",
                    ...
                    dbc.Col(
                        html.Div([
                            dash_table.DataTable(
                                ...
                                id='logros_tabla_proyectos_y_convenios_laboratorios',
                            ),
                        ], style={'paddingTop': '2%'})
                    )
                ]
...
@callback(
    Output("logros_tabla_proyectos_y_convenios_laboratorios", "data"),
    [Input("facultad_proyectos_y_convenios_laboratorios", "value"), Input("anio_proyectos_y_convenios_laboratorios", "value")])
def logros_alcanzados_proyectos_y_convenios_laboratorios(facultad, anio):
    if facultad or anio:
        if not anio:
            df = data
